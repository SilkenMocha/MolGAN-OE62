...
parser.add_argument("--dataset",   type=str, default="data/oe62_9nodes.sparsedataset", help="Ruta al dataset sparsedataset")
parser.add_argument("--pretrained_dir", type=str, default=None, help="Carpeta con modelo preentrenado")  # NUEVO argumento
...
# --- CARGAR MODELO PREENTRENADO si corresponde ---
if args.pretrained_dir is not None and os.path.exists(os.path.join(args.pretrained_dir, 'model.ckpt.index')):
    print(f"Cargando modelo preentrenado desde {args.pretrained_dir}")
    trainer.load(args.pretrained_dir)
...
# Puedes descomentar para guardar grillas de moléculas si lo necesitas:
#for i in range(1, 3):
#    mols = samples(data, model, session, model.sample_z(50), sample=True)
#    filename = f"{directory}/Output_{i}.png"
